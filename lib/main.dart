...
Future<String> fetchAiFeedback({
  required String codeSnippet,
  required String language,
  String? idToken,
}) async {
  final uri = Uri.parse('$backendBaseUrl/api/ai/feedback');
  ...
  if (response.statusCode == 200) {
    ...
    return (json['feedback'] as String?) ?? 'No feedback returned.';
  } else {
...
class _SubmitCodePageState extends State<SubmitCodePage> {
  ...
  if (_codeController.text.trim().isEmpty) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Please paste your code first')),
    );
    return;
  }
  ...
  try {
    ...
    final feedback = await fetchAiFeedback(
      codeSnippet: code,
      language: language,
    );
    ...
    setState(() => submitting = false);
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Failed to get AI feedback: $e')),
    );
  }
}
...
class ResultPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final aiSolution = sample
        ? 'for i in range(n):\n  # optimized version\n  print(i)'
        : (aiFeedback ?? '// No AI solution returned');

    final explanation = sample
        ? 'AI found an inefficiency in loop and suggested ...'
        : (aiFeedback ?? 'AI explanation placeholder');
